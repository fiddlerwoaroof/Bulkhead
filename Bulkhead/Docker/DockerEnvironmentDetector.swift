...
enum DockerEnvironmentDetector {
    ...
    static func detectDockerHostPath() -> String? {
        for path in socketLocations where isSocketAccessible(path) {
            LogManager.shared.addLog("Found accessible Docker socket at: \(path)", level: "DEBUG", source: "docker-environment-detector")
            return path
        }

        LogManager.shared.addLog("No accessible Docker socket found in common locations", level: "ERROR", source: "docker-environment-detector")
        return nil
    }
    ...
    private static func isSocketAccessible(_ path: String) -> Bool {
        ...
        guard fileManager.fileExists(atPath: path) else {
            LogManager.shared.addLog("Socket file does not exist at: \(path)", level: "ERROR", source: "docker-environment-detector")
            return false
        }
        ...
        do {
            ...
            LogManager.shared.addLog("Socket exists but is not accessible at \(path): \(error.localizedDescription)")
            return false
        }
    }
}
